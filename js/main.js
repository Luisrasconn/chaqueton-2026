  ...
  if (typeof AFRAME !== 'undefined') {
    ...
    var MANUALS = {
      'BANDA TRANSPORTADORA': [
        'MANUAL DE OPERACIÓN',
        '1. Verificar que la banda esté limpia antes de encender.',
        '2. Ajustar velocidad según la pieza (perilla lateral).',
        '3. No sobrepasar 10 piezas/minuto.',
        '4. En caso de atasco, presionar paro de emergencia.',
        '5. Lubricar rodillos cada 40 horas de operación.',
        '6. Revisar tensión de la banda semanalmente.'
      ],
      'BRAZO ROBOTICO': [
        'MANUAL DE OPERACIÓN',
        '1. Encender el controlador principal (botón verde).',
        '2. Verificar que no haya obstáculos en su área de trabajo.',
        '3. Seleccionar programa de ensamble en la interfaz.',
        '4. La carga máxima es de 5 kg — no exceder.',
        '5. Mantener una distancia de seguridad de 1 metro.',
        '6. En caso de error, revisar el código en el panel HMI.'
      ],
      'CONTENEDOR DE PIEZAS': [
        'MANUAL DE OPERACIÓN',
        '1. Las piezas tipo A (plástico) van en contenedor amarillo.',
        '2. Las piezas tipo B (metal) van en contenedor rojo.',
        '3. No mezclar tipos en el mismo contenedor.',
        '4. Peso máximo por contenedor: 500 unidades.',
        '5. Reabastecer cuando el nivel baje al 20%.',
        '6. Reportar piezas dañadas al supervisor.'
      ],
      'ROBOT SOLDADOR': [
        'MANUAL DE OPERACIÓN',
        '1. Usar careta de soldar con filtro adecuado.',
        '2. Verificar temperatura del arco (850°C ideal).',
        '3. Limpiar la punta del electrodo antes de cada uso.',
        '4. Tiempo por pieza: 12 segundos.',
        '5. Revisar cables de alimentación antes de operar.',
        '6. Mantener el área libre de materiales inflamables.'
      ],
      'ESCANER 3D': [
        'MANUAL DE OPERACIÓN',
        '1. Encender el escáner y esperar calibración (30 seg).',
        '2. Colocar la pieza en el centro de la plataforma.',
        '3. El escaneo toma aproximadamente 5 segundos.',
        '4. Revisar resultados en la pantalla táctil.',
        '5. Limpiar la lente con paño suave cada turno.',
        '6. Calibrar semanalmente con el patrón de prueba.'
      ],
      'EMPAQUETADORA': [
        'MANUAL DE OPERACIÓN',
        '1. Seleccionar el tipo de empaque en la pantalla.',
        '2. Ajustar temperatura del termoencogible (180°C).',
        '3. Colocar el producto en la banda de entrada.',
        '4. Ciclo: 8 segundos por paquete.',
        '5. Capacidad máxima: 120 unidades/hora.',
        '6. Limpiar residuos de plástico al finalizar el turno.'
      ]
    };
    ...
    AFRAME.registerComponent('machine-info', {
      ...
      init: function () {
        ...
        this.el.addEventListener('click', function () {
          if (self.manualPanel) { self.closeManual(); return; }
      ...
      showManual: function () {
        ...
        var manual = MANUALS[this.data.title];
        if (!manual) return;
        ...
        var lineCount = manual.length;
        var panelH = Math.max(2.5, 0.5 + lineCount * 0.28);

        var bg = document.createElement('a-plane');
        bg.setAttribute('width', '2.8');
        bg.setAttribute('height', panelH);
        bg.setAttribute('color', '#1a1a2e');
        bg.setAttribute('opacity', '0.95');
        bg.setAttribute('shadow', '');
        bg.setAttribute('radius', '0.08');
        ...
        header.setAttribute('position', '0 ' + ((panelH / 2) - 0.2) + ' 0.01');
        header.setAttribute('width', '2.6');
        header.setAttribute('height', '0.3');
        header.setAttribute('color', color);
        header.setAttribute('radius', '0.05');

        var titleText = document.createElement('a-text');
        titleText.setAttribute('value', 'MANUAL: ' + this.data.title);
        titleText.setAttribute('position', '0 ' + ((panelH / 2) - 0.2) + ' 0.02');
        titleText.setAttribute('color', 'white');
        titleText.setAttribute('align', 'center');
        ...
        manual.forEach(function (line, i) {
          var yPos = (panelH / 2) - 0.55 - (i * 0.28);
          var lt = document.createElement('a-text');
          lt.setAttribute('value', line);
          lt.setAttribute('position', '-1.1 ' + yPos + ' 0.02');
          lt.setAttribute('color', i === 0 ? '#f1c40f' : '#bdc3c7');
          lt.setAttribute('align', 'left');
          lt.setAttribute('width', '2.6');
          lt.setAttribute('scale', i === 0 ? '0.9 0.9 1' : '0.8 0.8 1');
          panel.appendChild(lt);
        });

        var closeY = -(panelH / 2) + 0.15;
        var closeBg = document.createElement('a-plane');
        closeBg.setAttribute('position', '0 ' + closeY + ' 0.01');
        closeBg.setAttribute('width', '2.6');
        closeBg.setAttribute('height', '0.2');
        closeBg.setAttribute('color', '#e74c3c');
        closeBg.setAttribute('radius', '0.05');
        closeBg.setAttribute('class', 'clickable');
        var self2 = this;
        closeBg.addEventListener('click', function () { self2.closeManual(); });

        var closeText = document.createElement('a-text');
        closeText.setAttribute('value', '✕  Cerrar manual');
        closeText.setAttribute('position', '0 ' + closeY + ' 0.02');
        closeText.setAttribute('color', 'white');
        closeText.setAttribute('align', 'center');
        closeText.setAttribute('width', '2.4');
        closeText.setAttribute('scale', '0.55 0.55 1');
        closeText.setAttribute('class', 'clickable');
        closeText.addEventListener('click', function () { self2.closeManual(); });
      ...
      open: function () {
        ...
        var header = document.createElement('a-plane');
        header.setAttribute('position', '0 1.15 0.01');
        header.setAttribute('width', '2.3');
        header.setAttribute('height', '0.3');
        header.setAttribute('color', color);
        header.setAttribute('radius', '0.05');
        ...
        titleText.setAttribute('position', '0 1.15 0.02');
        titleText.setAttribute('color', 'white');
        titleText.setAttribute('align', 'center');
        titleText.setAttribute('width', '2.2');
        ...
        // Close button
        var closeBg = document.createElement('a-plane');
        closeBg.setAttribute('position', '0 -1.05 0.01');
        closeBg.setAttribute('width', '2.3');
        closeBg.setAttribute('height', '0.2');
        closeBg.setAttribute('color', '#34495e');
        closeBg.setAttribute('radius', '0.05');
        ...
        var closeText = document.createElement('a-text');
        closeText.setAttribute('value', '✕  Cerrar');
        closeText.setAttribute('position', '0 -1.05 0.02');
        closeText.setAttribute('color', '#7f8c8d');
        closeText.setAttribute('align', 'center');
        closeText.setAttribute('width', '2.2');
        closeText.setAttribute('scale', '0.55 0.55 1');
        ...
        // Manual button
        var manualBg = document.createElement('a-plane');
        manualBg.setAttribute('position', '0 -1.35 0.01');
        manualBg.setAttribute('width', '2.3');
        manualBg.setAttribute('height', '0.2');
        manualBg.setAttribute('color', color);
        manualBg.setAttribute('radius', '0.05');
        manualBg.setAttribute('class', 'clickable');
        manualBg.addEventListener('click', function () { self2.showManual(); });

        var manualText = document.createElement('a-text');
        manualText.setAttribute('value', '📖  Ver manual');
        manualText.setAttribute('position', '0 -1.35 0.02');
        manualText.setAttribute('color', 'white');
        manualText.setAttribute('align', 'center');
        manualText.setAttribute('width', '2.2');
        manualText.setAttribute('scale', '0.55 0.55 1');
        manualText.setAttribute('class', 'clickable');
        manualText.addEventListener('click', function () { self2.showManual(); });

        panel.appendChild(manualBg);
        panel.appendChild(manualText);
